...
class Superjob(Engine):
    ...
    def get_request(self):
        ...
        my_auth_data = {"X-Api-App-Id": os.environ['SPR_JOB']}
    ...
    def get_vacancy(self):
        ...
        for i in range(len(self.request)):
            ...
            info = {
                ...
                'city': "(Город не указан)" if (self.request[i]['address']) == None else self.request[i]['address'][
                    'city'],
                'salary_from': 0.0 if (self.request[i]['salary'] == None or self.request[i]['salary']['from'] == 0 or
                                       self.request[i]['salary']['from'] == None) else self.request[i]['salary'][
                    'from'],
                'salary_to': "(Предельная зарплата не указана)" if (self.request[i]['salary'] == None or
                                                                    self.request[i]['salary']['to'] == None) else
                self.request[i]['salary']['to'],
                'currency': "(Валюта не указана)" if self.request[i][
                                                         'salary'] == None else f"{self.request[i]['salary']['currency']}",
        ...
        try:
            ...
            for i in range(len(self.request)):
                info = {
                    ...
                    'city': "(Город не указан)" if self.request[i]['town'] == None else self.request[i]['town'][
                        'title'],
                    'salary_from': 0.0 if self.request[i]['payment_from'] == 0 else self.request[i]['payment_from'],
                    'salary_to': "(Предельная зарплата не указана)" if (
                            self.request[i]['payment_to'] == 0 or self.request[i]['payment_to'] ==
                            None) else self.request[i]['payment_to'],
                    'currency': "(Валюта не указана)" if self.request[i]['currency'] == None else self.request[i][
                        'currency'],
...
class Vacancy(HHVacancy, SJVacancy, ):
    ...
    def del_json():
        '''
        Удаляет данные с файлом
        '''
        try:
            os.remove('all_vacancy.json')
            os.remove('hhvacancy.json')
            os.remove('sjvacancy.json')
